...
/*******************************************************
 * Waveform Data Structures for Serial-PIC-SPI
 *******************************************************/
typedef struct {
    uint8_t * waveType;
    uint8_t * frequency;
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            else
            {
                ...
                appData.tx_count = 0;
                if (count == 1){
                    //attributes.waveType = appData.readBuffer;
                    // open usart instance
                    appData.handleUSART0 = DRV_USART_Open(0, DRV_IO_INTENT_READWRITE|DRV_IO_INTENT_NONBLOCKING);
                    while (appData.tx_count < appData.numBytesRead){ // iterate over the array size
                        if(!DRV_USART_TransmitBufferIsFull(appData.handleUSART0)){ // wait until the buffer is transfered
                            DRV_USART_WriteByte(appData.handleUSART0, appData.readBuffer[appData.tx_count]); // write single byte char
                            appData.tx_count++; // next step
                        }
                    }
                ...
                appData.tx_count = 0;
                if (count == 2){
                    //attributes.frequency = appData.readBuffer;
                    appData.handleUSART0 = DRV_USART_Open(0, DRV_IO_INTENT_READWRITE|DRV_IO_INTENT_NONBLOCKING);
                    while (appData.tx_count < appData.numBytesRead){
                        if(!DRV_USART_TransmitBufferIsFull(appData.handleUSART0)){
                            DRV_USART_WriteByte(appData.handleUSART0, appData.readBuffer[appData.tx_count]);
                            appData.tx_count++;
                        }
                    }
                    DRV_USART_Close(appData.handleUSART0);
                ...
                appData.tx_count = 0;
                if (count == 3){
                    //attributes.amplitude = appData.readBuffer;
                    appData.handleUSART0 = DRV_USART_Open(0, DRV_IO_INTENT_READWRITE|DRV_IO_INTENT_NONBLOCKING);
                    while (appData.tx_count < appData.numBytesRead){
                        if(!DRV_USART_TransmitBufferIsFull(appData.handleUSART0)){
                            DRV_USART_WriteByte(appData.handleUSART0, appData.readBuffer[appData.tx_count]);
                ...
                appData.tx_count = 0;
                if (count == 4){
                    //attributes.offset = appData.readBuffer;
                    appData.handleUSART0 = DRV_USART_Open(0, DRV_IO_INTENT_READWRITE|DRV_IO_INTENT_NONBLOCKING);
                    while (appData.tx_count < appData.numBytesRead){
                        if(!DRV_USART_TransmitBufferIsFull(appData.handleUSART0)){
                            DRV_USART_WriteByte(appData.handleUSART0, appData.readBuffer[appData.tx_count]);
                ...
                /* Else echo each received character*/
                ...
                /* Turn off LEDs to indicate Rx is done with one input*/
                count++;
                if (count == 5) count = 1;
                USB_DEVICE_CDC_Write(USB_DEVICE_CDC_INDEX_0,
